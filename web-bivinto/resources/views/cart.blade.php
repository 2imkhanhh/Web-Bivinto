            ...
            @if($cartItems->count() > 0)
                ...
                <div class="row gx-5">
                    ...
                    <div class="col-12 col-lg-6 mb-5 mb-lg-0">
                        ...
                        @foreach($cartItems as $item)
                            ...
                            @endphp
                            <!-- Cart Item -->
                            <div class="d-flex cart-item pb-4 mb-4 border-bottom cart-item-row" data-id="{{ $item->id }}" data-price="{{ $item->product->price }}">
                                <div class="cart-item-img-wrap me-4 flex-shrink-0">
                                    <img src="{{ $imageUrl }}" alt="{{ $item->product->name }}" class="img-fluid object-fit-cover"
                                ...
                                <div class="cart-item-info flex-grow-1 d-flex flex-column justify-content-between">
                                    <div>
                                        ...
                                        @if($variantText)
                                            <p class="cart-item-variant text-muted mb-2" style="font-size: 0.85rem;">{{ $variantText }}</p>
                                        @endif
                                        <p class="cart-item-price fw-bold mb-0" style="font-size: 0.95rem;">{{ number_format($item->product->price, 0, ',', '.') }}đ</p>
                                    </div>
                                    <div class="d-flex justify-content-between align-items-start mt-3">
                                        <div class="d-flex flex-column gap-3">
                                            <div class="quantity-pill d-flex align-items-center justify-content-between rounded-pill border px-3 py-1"
                                                style="width: 100px;">
                                                <button type="button" class="btn p-0 border-0 btn-qty-minus" style="font-size:1rem;" onclick="updateCartQuantity({{ $item->id }}, -1)">-</button>
                                                <span class="fw-medium item-qty" style="font-size:0.95rem;">{{ $item->quantity }}</span>
                                                <button type="button" class="btn p-0 border-0 btn-qty-plus" style="font-size:1rem;" onclick="updateCartQuantity({{ $item->id }}, 1)">+</button>
                                            </div>
                                            <button type="button" class="btn p-0 border-0 text-dark d-flex align-items-center gap-2 cart-delete-btn" style="font-size:0.9rem;" onclick="removeCartItem({{ $item->id }})">
                                        ...
                                        <div class="cart-item-total fw-bold mt-1" style="font-size: 0.95rem;">
                                            Tổng: <span class="item-total-price">{{ number_format($itemTotal, 0, ',', '.') }}</span>đ
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                <div class="col-12 col-lg-6 ps-lg-5">
                        ...
                        <div class="order-summary-container mb-5">
                            ...
                            <div class="bg-light rounded px-3 py-4 border-0" style="background-color: #F6F6F6 !important;">
                                <div class="d-flex justify-content-between align-items-center mb-3">
                                    ...
                                    <span class="fw-bold" style="font-size: 0.9rem;" id="summary-total-items">{{ $totalItems }} sản phẩm</span>
                                </div>
                                <hr class="my-3 border-secondary border-opacity-25">
                                <div class="d-flex justify-content-between align-items-center mb-3">
                                    <span class="text-muted" style="font-size: 0.9rem;">Tạm tính</span>
                                    <span class="fw-medium" style="font-size: 0.95rem;"><span id="summary-subtotal">{{ number_format($subtotal, 0, ',', '.') }}</span>đ</span>
                                </div>
                                <div class="d-flex justify-content-between align-items-center mb-3">
                                ...
                                <div class="d-flex justify-content-between align-items-center mt-2">
                                    ...
                                    <span class="fw-bold" style="font-size: 1.1rem;"><span id="summary-total">{{ number_format($subtotal, 0, ',', '.') }}</span>đ</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="shipping-info-box mb-5">
                            <input type="text" class="form-control custom-input mb-3" placeholder="Họ và tên*" value="{{ auth()->user()->name ?? '' }}">
                        ...
                        <div class="payment-method-box mb-5">
                            <label class="payment-box border rounded px-4 py-3 d-flex align-items-center gap-3 w-100 m-0 cursor-pointer">
                                <input class="form-check-input mt-0 custom-radio" type="radio" name="paymentMethod" checked
                        ...
                        <!-- Checkout Button -->
                        <button class="btn btn-dark w-100 rounded-pill py-3 fw-medium" style="font-size: 1.05rem;">Đặt Hàng</button>
                    </div>
                </div>
            @else
