...
@section('content')
    <div class="order-detail-page py-4">
        <div class="container" style="max-width: 1000px;">
            <div class="mb-3">
                @if (auth()->check())
                    <a href="/don-hang" class="text-decoration-none text-dark"><i class="fa-solid fa-chevron-left me-2"></i>
                        Trở lại danh sách</a>
                @else
                    <a href="/tra-cuu" class="text-decoration-none text-dark"><i class="fa-solid fa-chevron-left me-2"></i> Trở
                        lại</a>
                @endif
            </div>
            ...
            <div class="order-detail-card d-flex justify-content-between align-items-center header-card">
                <div>
                    ...
                    <div class="text-muted" style="font-size: 0.9rem;">Đặt ngày:
            ...
            @if ($order->status !== 'cancelled')
                <div class="order-detail-card">
                    <div class="step-progress">
                        <div
                            class="step-item {{ in_array($order->status, ['pending', 'confirmed', 'shipping', 'completed']) ? 'active' : '' }}">
                            <div class="step-icon"><i class="fa-solid fa-file-invoice"></i></div>
                            <div class="step-label">Chờ Xác Nhận</div>
                        </div>
                        <div
                            class="step-item {{ in_array($order->status, ['confirmed', 'shipping', 'completed']) ? 'active' : '' }}">
            ...
            <div class="order-detail-card">
                ...
                <div class="ms-4">
                    ...
                    <div class="text-muted mb-1">{{ $order->ward }}, {{ $order->district }}, {{ $order->province }}
                    </div>
                    @if ($order->note)
            ...
            <div class="order-detail-card">
                ...
                @foreach ($order->items as $item)
                    ...
                    <div class="d-flex align-items-center mb-3 pb-3 {{ !$loop->last ? 'border-bottom' : '' }}">
                        <img src="{{ $imageUrl }}" class="order-item-img rounded me-3"
                        ...
                        <div class="flex-fill">
                            ...
                            @endif
                            <div class="fw-medium mt-1">{{ number_format($item->price, 0, ',', '.') }}đ x
                                {{ $item->quantity }}</div>
                        </div>
                ...
                <div class="row justify-content-end mt-4 pt-3 border-top">
                    <div class="col-12 col-md-5">
                        ...
                        <div class="d-flex justify-content-between mt-3 pt-3 border-top align-items-center">
                            ...
                            <span
                                class="fs-4 fw-bold text-danger">{{ number_format($order->total_amount, 0, ',', '.') }}đ</span>
                        </div>
                    </div>
                </div>
